...
public class GenericMethods {
	...
	public boolean verifyListofElementTextareSorted(By path, String Note) {
		...
		try {
			...
			if (actualText.stream().sorted().collect(Collectors.toList()).equals(actualText)) {
				logger.LogPass(driver, "The List of Text fectched are in Sorted order" + Note
						+ "\n Actual Text Fetched: " + actualText);
				return true;
			} else {
				logger.LogFail(driver, "The List of Text fectched are NOT in Sorted order" + Note
						+ "\n Actual Text Fetched: " + actualText);
				return false;
			}
				...
				| NoSuchElementException exctemp) {
			logger.LogFail(driver, "Failed to Validate if the List of Text elements are in Sorted order : " + Note
					+ " got the Exception message " + exctemp);
			return false;
		} catch (Exception exctemp) {
			logger.LogFail(driver, "Failed to Validate if the List of Text elements are in Sorted order : " + Note
